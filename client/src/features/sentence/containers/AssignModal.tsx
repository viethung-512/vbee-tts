...
interface AssignSentenceField {
  checker: User | string;
  count?: number;
  type?: SentenceType;
}

const getDefaultValue = (isAutoAssign: boolean): AssignSentenceField => {
  if (isAutoAssign) {
    return {
      checker: '',
...
const AssignModal: React.FC = () => {
  ...
  const {
    ...
    setSearchTerm,
  } = useAutoComplete(userAPI.getUsers, modalType !== 'AssignSentenceModal');
  const {
    control,
    formState,
    errors,
    ...
    reset,
    setError,
  } = useForm<AssignSentenceField>({
    mode: 'onChange',
    defaultValues: getDefaultValue(Boolean(modalProps?.isAutoAssign)),
  });
  const { doRequest: assignSentences, loading } = useMutation<
    AssignSentenceField,
    Sentence[]
  >(sentenceAPI.assignSentences, setError, () => {
  ...
  const submitForm = handleSubmit(async values => {
    ...
    if (modalProps && modalProps.isAutoAssign) {
      ...
      args.ids = modalProps.sentenceIds;
    }

    assignSentences(args);
  });

  return (
    <ModalWrapper
      modalType='AssignSentenceModal'
      closeable={true}
